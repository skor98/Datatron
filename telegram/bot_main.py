...
def get_queries_logs(message):
    ...
    try:
        ...
        if time_span:
            ...
            logs = logsRetriever.get_log(
                kind='queries', user_id=message.chat.id)
        ...
        if logs:
            ...
            bot.send_message(
                message.chat.id, constants.MSG_LOG_HISTORY_IS_EMPTY)
    except Exception as err:
...
def get_session_logs(message):
    ...
    try:
        ...
        if time_span:
            ...
            logs = logsRetriever.get_log(
                kind='session', user_id=message.chat.id)
...
def leave_feedback(message):
    try:
        ...
        if feedback:
            ...
            bot.send_message(
                message.chat.id, constants.MSG_WE_GOT_YOUR_FEEDBACK)
        else:
            bot.send_message(
                message.chat.id,
...
def what_cube_handler(message):
    ...
    try:
        ...
        for ind, elem in tuple(enumerate(clf.predict_proba(req)))[:3]:
            ...
            text_to_send += "{}. {} -> *{}%*\n".format(
                ind + 1, cube_name, round(proba * 100, 2))
...
def voice_processing(message):
    ...
    try:
        ...
        bot.send_message(
            message.chat.id, constants.ERROR_CANNOT_UNDERSTAND_VOICE)
...
def admin_notify(text, crit=False):
    ...
    for admin_id in SETTINGS.TELEGRAM.ADMIN_IDS:
        try:
            ...
            logging.critical(
                "Админ {} недоступен для отправки сообщения!".format(admin_id))
...
def process_cube_questions(
    ...
    if cube_result.status:
        ...
        if not is_input_text:
            user_request = '*Ваш вопрос*\n"{}"\n\n'
            user_request = user_request.format(
                cube_result.feedback['user_request'])
...
def process_minfin_questions(
        ...
        if isinstance(minfin_result.picture_caption, list):
            ...
            with open('data/minfin/img/{}'.format(minfin_result.picture), 'rb') as picture:
                bot.send_photo(message.chat.id, picture,
                               caption=minfin_result.picture_caption)
    ...
    if SETTINGS.TELEGRAM.ENABLE_ADMIN_MESSAGES:
        bot.send_message(
            message.chat.id, 'Score: {}'.format(minfin_result.score))
...
def bot_info_str(html=False):
    template = (
        'Bot <b>{bi.first_name}</b> '
        'at <a href=https://t.me/{bi.username}>@{bi.username}</a><br>\n'
        'API Token: {token}'
    )
    if not html:
        template = re.sub(r'<.*?>', '', template)

    return template.format(bi=bot_info, token=bot.token)


def webhook_info_str(html=False):
    wh_info = bot.get_webhook_info()

    current = wh_info.url != ''
    default = SETTINGS.TELEGRAM.ENABLE_WEBHOOK

    if default == current:
        template = 'Webhook is {}active (default setting)'.format(
            '' if current else 'in')
    else:
        template = 'Webhook is {}activated ({}active by default)'.format(
            '' if current else 'de',
            '' if default else 'in'
        )
    if current:
        template += '<br>\nWebhook URL: <a href={wi.url}>{wi.url}</a><br>\n'
        errtime = getattr(wh_info, 'last_error_date')
        if isinstance(errtime, int):
            wh_info.errtime = asctime(localtime(errtime))
            template += ('It seems that is is not working properly<br>\n'
                         'Last error at {wi.errtime}: '
                         '<i>{wi.last_error_message}</i>')
    if not html:
        template = re.sub(r'<.*?>', '', template)
    return template.format(wi=wh_info)
