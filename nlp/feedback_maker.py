    ...
    def _preprocess_fb(cube, verbal_feedback):
        ...
        if cube == 'CLMR02':
            ...
            if 'месяц' not in res:
                ...
                month = norm_months.index(res['месяц']) + 1
            if month == 12:
                month = 1
                year += 1
            else:
                month += 1
            res['дата'] = '01.{}.{}'.format(
                month if month >= 10 else '0' + str(month),
                year
            )
            ...
            if 'год' not in res:
                res['год'] = '2017'
            res['дата'] = '{} {} года'.format(res.get('месяц'), res.get('год'))
        elif 'год' in res:
            res['дата'] = '{} год'.format(res.get('год'))
...
class CubeMasks(object):
    ...
    Месяцу (если он есть) и году соответствует код "дата" (нечто вида "март 2014 года")
    ...
    Пример маски:
    '{^показатели*nomn|Годовые доходы} {?бюджета ?территория*gent|федерального бюджета}{? в категории ?группа*nomn} в {дата*loc2|прошлом году}{?: ?мера*nomn}'
    '''

    _common_cube_prefix = '{куб*nomn|Прочие данные}. '

    _base_cube_masks = {
        'CLDO01': 'Оперативные данные по {показатель*datv|всем уровням бюджета}{?: ?мера*nomn}',
        'CLDO02': '{^показатели*nomn?, о?|О}перативные данные по {уровень*datv|всем уровням бюджета}{? ?территория*gent}{?: ?мера*nomn}',
        'CLMR02': '{^показатели*nomn|Госдолг РФ} по состоянию на {дата}{?: ?мера*nomn}',
        'EXDO01': '{^показатели*nomn|Расходы} {уровень*gent|всех уровней бюджета}{? ?территория*gent}{? на ?раздел*accs}{?: ?мера*nomn}',
        'EXYR03': '{^показатели*nomn|Годовые расходы} {уровень*gent|всех уровней бюджета}{? ?территория*gent}{? на ?раздел*accs} в {дата*loc2|прошлом году}{?: ?мера*nomn}',
        'FSYR01': '{^показатели*nomn|Финансирование} {уровень*gent|всех уровней бюджета}{? ?территория*gent} в {дата*loc2|прошлом году}{?: ?мера*nomn}',
        'INDO01': '{^показатели*nomn|Доходы} {уровень*gent|всех уровней бюджета}{? ?территория*gent}{? за счёт ?группа*gent}{?: ?мера*nomn}',
        'INYR03': '{^показатели*nomn|Годовые доходы} {уровень*gent|всех уровней бюджета}{? ?территория*gent}{? за счёт ?группа*gent} в {дата*loc2|прошлом году}{?: ?мера*nomn}'
    }
